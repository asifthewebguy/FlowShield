          ...
          <div className={`mt-12 lg:mt-0 animate-reveal animate-reveal-delay-2 ${isVisible ? 'visible' : ''}`}>
            <div className="bg-surface-1 border border-surface-3 rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-surface-3 flex items-center gap-2">
                <svg className="w-5 h-5 text-accent-400" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                ...
                {leaderboard.map((entry) => (
                  ...
                  >
                    ...
                    <span className={`flex-1 text-sm ${entry.isYou ? 'text-primary-400 font-medium' : 'text-gray-300'}`}>
                      {entry.name}
                      {entry.isYou && (
                        <span className="ml-2 text-xs bg-primary-900/40 text-primary-400 px-1.5 py-0.5 rounded">
                          You
                        </span>
                      )}
                    </span>
