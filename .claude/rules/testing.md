...
| Suite | Count | Command |
|-------|-------|---------|
| Vitest unit tests (web) | **312** across 31 files | `cd web-app && npm test` |
...
## Web Unit Tests (312 Vitest, 31 files)
...
`src/app/api/`:
`coach/advice` 8 · `tasks` 8 · `auth/callback-exchange` 6 · `auth/login` 6 · `auth/reset-password` 6 ·
...
1. `cd web-app && npm test` — 312 passing, no Redis stderr noise
