...
| Suite | Count | Command |
|-------|-------|---------|
| Vitest unit tests (web) | **277** across 28 files | `cd web-app && npm test` |
...
| Desktop v3 (Tauri, Rust) | `#[cfg(test)]` modules in 25 files | `cd desktop-app-v3/src-tauri && cargo test --lib` |
| k6 load test | — | GitHub Actions `workflow_dispatch` or weekly schedule |

## Web Unit Tests (277 Vitest, 28 files)

`src/lib/`:
`schemas` 35 · `insights` 26 · `productivity` 21 · `coach-quota` 18 · `subscription` 16 ·
...
`src/app/api/`:
`coach/advice` 8 · `auth/callback-exchange` 6 · `auth/login` 6 · `auth/reset-password` 6 ·
`cron/expire-subscriptions` 6 · `auth/request-verification` 6 · `auth/forgot-password` 5 ·
`user/password` 5 ·
`auth/logout-all` 4 · `projects/[id]` 4 · `sessions/[id]/auto-end` 4 · `user/delete` 4 ·
`activity/sync` 3 · `sessions` 3
...
1. `cd web-app && npm test` — 277 passing, no Redis stderr noise
